...
#define RAINBOW_LIB_PATH "librainbow.so"
typedef bool (*RainbowCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_rainbow_core_main() {
   void* librainbow = dlopen(RAINBOW_LIB_PATH, RTLD_LAZY);
   if (!librainbow) {
      fprintf(stderr,"Failed to load \"librainbow.so\"\n");
      char* error;
   ...
   auto core_main = (RainbowCoreMain) dlsym(librainbow,"rainbow_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"rainbow_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
