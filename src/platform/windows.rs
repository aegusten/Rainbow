...
pub fn try_kill_rustdesk_main_window_process() -> ResultType<()> {
    // Kill rainbow.exe without extra arg, should only be called by --server
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    log::info!("try kill rainbow main window process");
    ...
    bail!("failed to find rainbow main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_rainbow.log for more details.");
        } else {
